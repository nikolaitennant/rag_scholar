...
CTX_DIR   = "default_context"
INDEX_DIR = "faiss_store"
CHUNK_SZ  = 800                  # larger → fewer chunks / embeddings
CHUNK_OV  = 40
FIRST_K   = 12                   # search fewer candidates
FINAL_K   = 4
MAX_TURNS = 30
...
LOADER_MAP = {
    "pdf":  PyPDFLoader, "docx": safe_docx_loader, "doc": TextLoader,
    "pptx": UnstructuredPowerPointLoader, "csv": CSVLoader, "txt": TextLoader,
    "png": UnstructuredImageLoader, "jpg": UnstructuredImageLoader, "jpeg": UnstructuredImageLoader,
}
...
def ocr_bytes(b: bytes) -> str:
    try:
        return pytesseract.image_to_string(Image.open(io.BytesIO(b)), lang="eng", config="--psm 6")
    except Exception:
        return ""
...
FAST_MODE = st.sidebar.toggle("Skip rerank & BM25 (fast)", value=True)
...
if uploaded_docs:
    ...
    if new_docs:
        ...
        vs.save_local(INDEX_DIR)  # persist
        del new_docs
        gc.collect()
...
if query:
    ...
    msgs: List[Union[SystemMessage, HumanMessage]] = [SystemMessage(content=system_prompt)]
    if snippets:
        msgs.append(SystemMessage(content="Snippets:\n" + "\n\n".join(snippets)))
    ...
    with st.chat_message("assistant"):
        ...
        for chunk in client.chat.completions.create(
            model=LLM_MODEL,
            messages=[{"role": "system", "content": m.content} if isinstance(m, SystemMessage) else {"role": "user", "content": m.content} for m in msgs],
            temperature=0.0,
            max_tokens=800,
            stream=True,
        ):
    ...
    for sent in sent_tokenize(answer):
        s = sent.strip()
        if not ALPHA_PAT.search(s):
            continue
        if CITE_PAT.search(s):
            continue
        if len(s.split()) > 25 or any(k in s.lower() for k in LEGAL_KEYWORDS):
            uncited.append(s)
    if uncited:
