...
# Introductory info box (unchanged)
st.markdown("""
<div style='
    margin-bottom: 24px;
    padding: 26px 28px 20px 28px;
...
2. **Add a Fact**  
   - **Session-only:** prefix with `memo:`  
   - **Permanent:** prefix with `remember:`
...
new_fact = st.sidebar.text_input("Add fact (memo: or remember:)")
persona_input = st.sidebar.text_input("Set persona (role:)")

# ─── Fact / Persona Processing ────────────────────────────────────────────────
if new_fact:
    txt = new_fact.strip()
    if txt.lower().startswith("remember:"):
        fact = txt.split(":",1)[1].strip()
