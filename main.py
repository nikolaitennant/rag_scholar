...
def build_vectorstore(default_docs, default_index, dynamic_docs):
    if dynamic_docs:
        # clone default index to avoid mutation
        vs = copy.deepcopy(default_index)
        vs.add_documents(dynamic_docs)
        return vs
...
# Persist mode: save files into default_context for future sessions
...
dynamic_docs = session_docs + memory_docs
vector_store = build_vectorstore(default_docs, default_index, dynamic_docs)
retriever = vector_store.as_retriever()
llm = ChatOpenAI(api_key=api_key, model="gpt-4o-mini", temperature=0.0)
