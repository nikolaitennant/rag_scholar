...
# Sidebar Controls (instructions only)
st.sidebar.header("📂 Sidebar Controls")
st.sidebar.markdown("""
• Use the 📎 icon to upload PDF or TXT files inline below.  
• Or type in commands and questions in the chat bar.
""")

# ─── Load default documents & memory facts ─────────────────────────────────────
default_index = load_and_index_defaults()
memory_docs = [type("Doc", (), {"page_content": f})() for f in st.session_state.memory_facts]

# ─── Main Chat + Inline File Upload ─────────────────────────────────────────────
upload_col, chat_col = st.columns([1, 9], gap="small")
with upload_col:
    inline_files = st.file_uploader(
        ...
        help="Upload PDF/TXT files"
    )
with chat_col:
    user_input = st.chat_input(
        "Type a question or use `remember:`, `memo:`, `role:`…"
    )

# Build vector store with any inline uploads + memory docs
...
if user_input:
    ...
    # Fallback if no context or facts
    if not (docs or memory_docs or st.session_state.session_facts):
        st.warning("⚠️ Sorry, there is not enough information to answer your request.")
    else:
        resp = llm.invoke(messages)
        st.session_state.chat_history.append(("User", txt))
