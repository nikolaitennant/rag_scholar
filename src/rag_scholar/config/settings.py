...
class Settings(BaseSettings):
    ...
    use_hybrid_search: bool = Field(
        default=True, description="Enable hybrid (vector + keyword) search"
    )
    bm25_weight: float = Field(
        default=0.3, description="BM25 weight in hybrid search", ge=0.0, le=1.0
    )

    # Storage
    data_dir: Path = Field(default=Path("data"), description="Data directory path")
    index_dir: Path = Field(default=Path("indexes"), description="Index directory path")
    ...
    # UI Configuration
    ui_title: str = Field(
        default="RAG Scholar - Research Assistant", description="UI page title"
    )
    ui_page_icon: str = Field(
        default="📚", description="UI page icon (can be emoji or text)"
    )
    ...
    rate_limit_per_minute: int = Field(
        default=100, description="API requests per minute per session"
    )
    upload_limit_per_hour: int = Field(
        default=10, description="File uploads per hour per session"
    )
    max_concurrent_websockets: int = Field(
        default=5, description="Maximum concurrent WebSocket connections per IP"
    )
    ...
    max_session_history: int = Field(
        default=20, description="Maximum messages to keep in session history"
    )

    # Search Configuration
    max_search_results: int = Field(
        default=20, description="Maximum search results to return"
    )
